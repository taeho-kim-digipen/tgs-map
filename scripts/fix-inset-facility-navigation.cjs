...
// Put enlarged Hall 9 inset booths above the broad 9–11 hall hitboxes so taps
// inside Enlarged Map C / business / Selected 80 resolve to the detailed booth.
app=replaceOne(app,
...
// Make visible favorite labels tappable too. In the tiny enlarged-map areas,
// use a short code label so it cannot cover the neighboring main-hall drawing.
app=replaceOne(app,
...
// The compact selected-booth card keeps details behind View, but exposes route
// guidance directly so users do not have to open the detail sheet first.
app=replaceOne(app,
...
// Facility cards now include a real routing action.
app=replaceOne(app,
...
// Highlight the facility that is currently the navigation destination.
app=replaceOne(app,
